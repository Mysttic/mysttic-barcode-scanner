...
Its interface is in English; the tray menu has a **Language** submenu with
Polish as the second option.
...
|---|---|---|
| the window matches a profile | captures the scan and replays the macro | a toast: "Filled in: steps 4/4" |
...
4. **Step 3 — record the actions.** Click "Record", switch to the
...
Tray icon → **Profiles (manage)** (Profiles, manage): enabling and disabling a
profile, renaming it, correcting the **process and the window title pattern**,
previewing the macro steps and deleting. The *Open file* button
...
```json
{
  "name": "Employee card (ERP)",
  "enabled": true,
  "match": { "process": "erp", "titlePattern": "*Employee card*" },
  "parse": {
    "type": "delimited",
    "prefix": "PRC;",
    "separator": ";",
  ...
  "steps": [
    { "action": "field", "target": { "automationId": "txtFirstName" }, "value": "{firstName}", "mode": "type" },
    { "action": "field", "target": { "automationId": "cmbDepartment" }, "value": "{department}", "mode": "select" },
    { "action": "key", "key": "TAB" },
    { "action": "text", "value": "HR" },
    { "action": "click", "target": { "x": 120, "y": 380 } },
    { "action": "pause", "ms": 200 }
  ]
}
```
...
|---|---|
| `field` | type a value (a template with `{field}`) into the given control; `mode` is `type`, `select` (pick from a list) or `auto` |
| `text` | type text wherever the caret is |
| `key` | TAB, ENTER, ESC, F1-F12, arrows, HOME/END, DELETE, BACKSPACE |
| `click` | click a control or a point (coordinates relative to the window) |
| `pause` | wait the given number of milliseconds (for slow applications) |

Parsing types (`parse.type`): `delimited`, `regex` (groups, as in the device) and
...
The `settings` section of `profile.json`:

| Key | Default | Meaning |
|---|---|---|
| `scanGapMs` | 60 | the longest gap between characters still counted as a scan |
| `minFrameLength` | 3 | shorter frames are ignored |
| `stepPauseMs` | 40 | pause between macro steps (raise it for slow applications) |
| `verifyByReadback` | true | confirm each fill by reading the value back |
